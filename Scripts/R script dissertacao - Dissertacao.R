...
vote_state <- vote_state[,c('state','year','incumbent_party','winner_party'
                            ...
                            ,'incumbent_candidate_dm','challenger_running'
                            ,'incumbent_running')]
...
m1 <- lm(reelection_party ~ oil_deflated_std
          + unified_gov
          + incumbent_party
...
m2 <- lm(reelection_candidate ~ oil_deflated_std
          + unified_gov
          + incumbent_party
          + midterm_punishment
          + State 
          ,data = subset(df_analysis, incumbent_candidate_dm == 1))
...
screenreg(
  ...
  ci.force = TRUE,
  custom.header = list(
    "Model 1: Party re-election" = 1:1,
    "Model 2: Candidate re-election" = 2:2
  ),
...
length(m1$coefficients) # número de parâmetros estimados
nobs(m1)
VIF(m1)
length(m2$coefficients) # número de parâmetros estimados
nobs(m2)
VIF(m2)
...
m3 <- lm(savings_i ~ oil_deflated_std*BSF_implemented
...
m4 <- lm(totexp_i ~ oil_deflated_std*BSF_implemented
          + gubernatorial_election_year_dm
          + incumbent_party
          + unified_gov
          + factor(State)
...
screenreg(list(m3,m4),
          ...
          ci.force = TRUE,
          custom.header = list(
            "Model 3: Real savings per capita" = 1:1,
            "Model 4: Real expenditures per capita" = 2:2
          ),
          custom.model.names = c(
            "OLS SE","OLS SE"
          ),
          ...
          custom.gof.rows = list(
            "State FE" = c("Yes", "Yes"),
            "State Linear Trends" = c("Year","Yes"),
            "SE" = c("OLS", "OLS")
          )
...
# Check multicollinearity and number of observations/parameters 
length(m3$coefficients) # número de parâmetros estimados
nobs(m3)
VIF(m3)
length(m4$coefficients) # número de parâmetros estimados
nobs(m4)
VIF(m4)
...
m5 <- lm(reelection_party ~ oil_deflated_std*BSF_implemented
          + unified_gov
          + incumbent_party
          + incumbent_candidate_dm
          + midterm_punishment
          + State 
          ,data = df_analysis)
m6 <- lm(reelection_candidate ~ oil_deflated_std*BSF_implemented
          + unified_gov
          + incumbent_party
          + midterm_punishment
          + State 
...
# Check multicollinearity and number of observations/parameters 
length(m5$coefficients) # número de parâmetros estimados
nobs(m5)
VIF(m5)
length(m6$coefficients) # número de parâmetros estimados
nobs(m6)
VIF(m6)
...
########################################################
p_vals <- lapply(list(m1, m2, m3, m4, m5, m6), function(x) texreg::extract(x)@pvalues)

# Make me a single table with all models
screenreg(
  ...
  custom.gof.rows = list(
    ...
    "State Linear Trends" = c("No","No","Yes","Yes","No","No"),
    "SE" = c("OLS", "OLS", "OLS", "OLS", "OLS","OLS")
  ))
